...
func main() {
	...
	cpuPlot.Data[0] = make([]float64, 50)
	...
	memPlot.Data[0] = make([]float64, 50)
	...
	netPlot.Data[0] = make([]float64, 50)
	netPlot.Data[1] = make([]float64, 50)
	...
	for {
		...
		case <-ticker:
			...
			cpuPlot.Data[0] = append(cpuPlot.Data[0][1:], percent[0])
			...
			sent := float64(netIO[0].BytesSent-prevNetIO[0].BytesSent) / 1024
			recv := float64(netIO[0].BytesRecv-prevNetIO[0].BytesRecv) / 1024
			prevNetIO = netIO

			netPlot.Data[0] = append(netPlot.Data[0][1:], math.Round(sent*10)/10)
			netPlot.Data[1] = append(netPlot.Data[1][1:], math.Round(recv*10)/10)
